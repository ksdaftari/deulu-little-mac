...
%todo: top level summary 6 bullet points, eg side B goes over most attack options (grab, down smash, minecart, etc)

%%% DK

%todo: see if grab from up smash on shield still hits, use grab distances to see which grab distance is minimum which cann then inform characters
...
%todo add studies of chess players on how many calories and amount of complex focused activity mind can do in one sitting
%todo: detial out how muscle fibers break then regrow at rest, so rest and fueling is actually where improvement done
%todo: SMART goals, what examples of those concrete goals, better to actually just document my weekly game plans so people can see through practice
%todo: write sub goals of character and unique play style (goes into why explaining why chose mac), where want to "least amount of inputs to win", with a game that is so into tech and how many complex inputs can do, i strive for less complexity and the minimal simplicity to consistently win, at times my mac may just look slow and methodical

1. I avoid:
  1. Prolongged periods of auto-piloting and winning lot but against lower competition (as I am not learning even though can be fun). I may every once and while just for fun and connect with community
  2. Over grinding (ie putting into too many continous hours of play in a day) is counter productive. prioritize fewer smaller focused sessions
2. Ensure get good sleep and rest from game to ensure mind can process what learned into memory. If don't the progress you felt today will not be felt tomorrow, it will be forgotten.
3. 
